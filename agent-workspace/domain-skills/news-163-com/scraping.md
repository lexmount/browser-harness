Field-tested on 2026-07-06 — 网易新闻站内搜索(news.163.com / www.163.com/search)：抓关键词搜索结果的标题/来源/日期，并从详情页取完整发布时间。
...
- **首次 http_get 可能返回截断/缓存的短响应**（我第一次只拿到 8 条 / 87KB，重取即得完整 50 条 / 278KB）。校验 `len(body)` 与 `keyword_new` 计数，偏少就重试一次。
